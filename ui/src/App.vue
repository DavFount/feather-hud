<script setup>
  ...
  // (PGUP close) Config.UI.hotkey ("PGUP", feather-hud/config.lua) only ever
  // reaches the game while the menu is closed -- once open, SetNuiFocus hands
  // keyboard input to this browser (ui.lua's 'updatestate' callback), so the
  // Lua-side KeyPressAPI listener never sees a second press to close it.
  // Caught here instead, reusing the same close path as the X button.
  const onKeydown = (event) => {
    if (event.code !== "PageUp" || !visible.value) return;
...
<template>
  <Transition name="bounce">
    <div id="content" class="centerit text-white mx-auto px-10 pt-12 max-w-sm rounded overflow-hidden aspect-[1/2]" v-if="visible || devmode">
      ...
      <div class="absolute bottom-10 left-0 w-full h-16">
        ...
        <div style="width: 90%" class="grid max-w-lg grid-cols-4 mx-auto font-medium">
          <router-link active-class="active" to="/" class="inline-flex flex-col items-center justify-center px-5 group">
            <svg
              class="w-8 h-8 mb-2 text-gray-500 group-hover:text-red-400"
              aria-hidden="true"
              fill="currentColor"
          ...
          <router-link active-class="active" to="/settings" class="inline-flex flex-col items-center justify-center px-5 group">
            <svg
              class="w-8 h-8 mb-2 text-gray-500 group-hover:text-red-400"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="absolute left-6 top-4 text-2xl text-white cursor-pointer hover:text-red-400" @click="closeApp">&times;</div>
    </div>
  </Transition>
</template>
